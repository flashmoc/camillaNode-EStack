(() => {
  'use strict';
  document.documentElement.dataset.prototypePage = 'control';
  ...
  function setGain(key, raw, preview = false) {
    const master = key === 'master'; const min = master ? -50 : -60; const max = master ? 0 : 6; const next = Number(clamp(raw, min, max).toFixed(1));
